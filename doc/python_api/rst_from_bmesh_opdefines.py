...
def main():
    with open(FILE_OP_DEFINES_C, 'r', encoding="utf-8") as fsrc:
        blocks = []
    ...
    with open(OUT_RST, 'w', encoding="utf-8") as fout:
        fw = fout.write
        fw(HEADER)
                ...
                for i in args_index:
                    ...
                    if tp == BMO_OP_SLOT_FLT:
                        ...
                        if tp_sub == BMO_OP_SLOT_SUBTYPE_INT_ENUM:
                            ...
                            tp_str = f"enum in {enums}, default {default_value}"
                        elif tp_sub == BMO_OP_SLOT_SUBTYPE_INT_FLAG:
                            default_value = 'set()'
                            tp_str = f"set of flags from {enums}, default {default_value}"
                        else:
                            tp_str = "int"
                            default_value = '0'
...
def arg_name_with_default(arg):
    ...
    return name if default_value is None else f'{name}={default_value}'
